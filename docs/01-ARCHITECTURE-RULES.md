...
> المرجع الحاكم: `docs/11-DEFINITION-OF-DONE.md` (بوابة G1–G8) و`docs/15-ROADMAP.md`
> (الترتيب الملزم). `docs/COMPLETE_PROJECT_PLANS.md` أرشيف تاريخي لمعرّفات المهام
> (P?-T??) — حالته غير موثوقة، انظر `docs/16-KNOWN-ISSUES.md`.
...
**قاعدة:** كل استعلام SQL يعيش في `models/` (أو عبر خدمة في `lib/services/`
تستدعي نموذجاً). تحقّق F-5A → F-5D: `CompetitionController` و`AdminController`
وصفحتا `profile-page` و`live/main` بلا SQL مباشر (صفر `.prepare(`) — أي SQL
جديد فيها مخالفة تُرفض. الاستثناء الوحيد الموثّق: `FollowModel` نُقل كما هو من
داخل `UserController` (F-5B) **دون** وراثة `BaseModel` — إعادة هيكلته خارج النطاق،
لا "تصلحه" بإضافة وراثة من عندك (انظر `docs/16-KNOWN-ISSUES.md`).
...
1. اقرأ `AGENTS.md` + `docs/15-ROADMAP.md` وحدد المرحلة/البند (معرّفات P?-T??
   التاريخية في `docs/COMPLETE_PROJECT_PLANS.md` للمرجع فقط، لا تُنشئ خطة جديدة).
2. حدّث حالتها في `PLAN-STATUS.md` بالرمز الصادق (`🔧` جارية / `🧪` مُتحقَّقة
   يدوياً بلا اختبار آلي) — `✅` فقط بعد اجتياز G1–G8 في `docs/11-DEFINITION-OF-DONE.md`.
3. نفذ التعديل وفق القواعد أعلاه.
4. اختبر وفق "اختبار القبول" الخاص بالمهمة (اختبار آلي أحمر أولاً حيث ينطبق).
5. سجّل في `WORKLOG.md` + حدّث الحالة (لا تُعلن `✅` بلا دليل G1–G8).
