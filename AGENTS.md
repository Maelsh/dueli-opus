...
3. `docs/16-KNOWN-ISSUES.md` — تصحيحات ملزمة (ترقيم 0012 المكرر، افتراضات ثبت خطؤها، ما ليس مصدر حقيقة) — اقرأه قبل افتراض أي شيء قديم — وسياسة الـ dot-folders/artifacts في `docs/17-DOT-FOLDERS-POLICY.md` (تاريخية، ليست مصادر حقيقة، ممنوع البناء عليها أو إنشاء جديد منها)
...
- **تغيير المستودع (F-10):** ممنوع إضافة أي ملف/مجلد خارج نطاق مهمتك دون إذن صريح موثّق — يشمل ملفات التخطيط ونسخ الخطط ومجلدات Agent artifacts والتقارير المؤقتة وملفات الاختبار المؤقتة والأدوات الشخصية وأي dot-folder جديد (`.agent/` `.claude/` `.gemini/` وأشباهها). الملف الجديد مسموح فقط إن كان داخل النطاق ومبررًا بوضوح في تقرير المهمة أو بموافقة صريحة. التفاصيل: `docs/18-REPOSITORY-CHANGE-POLICY.md` (تعميم لسياسة F-9 في `docs/17-DOT-FOLDERS-POLICY.md`).
