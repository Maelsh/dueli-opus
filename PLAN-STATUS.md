...
- 🔧 منفَّذ محلياً (من الرأس `f280ee6` على `feat/money-withdrawals` الذي يحمل 8.A–8.D). النطاق: تبرع المشاهد لمتنافس بأثر مالي حصري عبر `LedgerService` — لا رصيد موازٍ، لا مسار مالي ثانٍ، لا تعديل لسياسة 8.A/8.B/8.C ولا لـ`LedgerService`، لا CI/dependencies، لا Production deployment/migration/merge.
  ...
  - **الاختبارات**: `tests/api/donations.test.ts` (12 عبر Hono الحقيقي — الثمانية المطلوبة + الاتجاه المعاكس + i18n + جزئي/تكرار الاسترداد 8b/8c؛ سُلّمت حمراء أولاً ثم خضراء). الصيانة: `schema-contract` (23) و`fake-d1` (الأعمدة الجديدة).
  - **التحقق**: `npm test` 438/438 ✅ + `tsc` ✅ + `build` ✅ + `db:reset` ✅ + تكامل `schema-contract` 16/16 ✅ + تكامل `ledger` 15/15 ✅. الحالة 🔧 (تحقق محلي) ريثما تكتمل G1–G8 بالمراجعة الخارجية — بلا دمج.
