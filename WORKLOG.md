## 2026-09-26 — PR #68 final B6 remediation: real-IMG click + real messages DOM (branch `fix/ui-auth-stabilization-batch`)

- **OLD HEAD / NEW HEAD**: `e015e1a` → see PR #68 head. Scope: B6 only. SW, CSP, B1–B5, PR66/#67 and the B7 deferral untouched.
- **Why the previous pass was a false green (both causes were mine, not the REMOTE's).**
  1. **The test clicked the wrong node.** The card avatar test dispatched its click on the wrapper `<span role="link">`. In the real DOM a user clicks the `<img>` *inside* that span — and that `<img>` carries `data-csp-on="error" data-csp-fn="__fallbackSrc"`. The dispatcher resolved handlers with `closest('[data-csp-on][data-csp-fn]')`, which is type-blind: a click landing on the image selected the image's **error** handler, the enclosing `__navigateProfile` never ran, nothing called `preventDefault`, and the click fell straight through to the parent `/competition/:id` link. The markup was always correct; the *dispatch semantics* were wrong, and my test had been asserting on the wrapper, so it could not see it.
  2. **The messages fix targeted an element that does not exist.** I had modified `client/ui/MessagingUI.ts`, which renders into `#conversations-list`. That id appears nowhere in the app. The canonical conversation list is the page's own inline script writing into **`#conversationsList`** (`src/modules/pages/messages-page.ts`), and that row avatar was still a bare `<img>`. `window.MessagingUI` is in fact bound to a different class (`MessagesUI`), so `MessagingUI` is legacy on this surface.
- **Fix 1 — central, no surface hack.** `dispatch()` now resolves the action in two passes via a new `findAction()` ancestor walk: first the nearest action bound to **this** event type, then any action (still needed by the keydown path, where the element is deliberately bound to `click`). So a click on an `<img data-csp-on="error">` now reaches the enclosing profile action, while a real `error` event still reaches `__fallbackSrc`. Implemented as an ancestor walk rather than a built selector string, so the C5 guard "every `data-csp-on` uses a delegated event type" still applies to source text.
- **Fix 2 — the real DOM surface.** The `#conversationsList` row avatar is now the same delegated profile action (inside the `other_username ?` branch, so a blank username still yields a plain, unlinked image). `MessagingUI.ts` was reverted: it is not the live surface, and leaving a second, dead renderer carrying the fix would have been exactly the "two competing renderers" trap. Row click still opens the conversation (unread badge, messages rendering, dropdown and `chatUserAvatar` untouched). No nested `<a>`; CSP and `role`/`tabindex`/`aria-label` preserved.
- **Tests** — `tests/ui/b6-avatar-click-navigation.test.ts` now builds the **real** card DOM (span + `<img data-csp-on="error">`) and covers: wrapper click → profile, **IMG click → profile**, `error` event → `__fallbackSrc` still rewrites `src`, card click outside the avatar → no profile navigation, Enter/Space → profile, native `<button>` not double-activated, non-username rejected. `tests/ui/b6-messages-page-avatar.test.ts` (new) renders the real `/messages` page through Hono, runs its real inline script against a stubbed `/api/conversations`, asserts the produced row markup, then drives the real dispatcher: avatar click → profile, row text click → `openConversation`. Both were RED first (IMG click asserted `[]`, row click asserted `[]`).
- `npm test` **733/733 ✅** · `npx tsc --noEmit` ✅ · `npm run build` ✅.
- **Untouched by design**: `ORDER BY RANDOM()` / ranking (B7 still deferred to the Lead/Owner), service-worker architecture, CSP policy, backend/session/schema/migration.


## 2026-09-26 — PR #68 blocker remediation: stale SW assets + B6 avatar click (branch `fix/ui-auth-stabilization-batch`)

- **OLD HEAD / NEW HEAD**: `b435ccc` → see PR #68 head. Base unchanged (`ff3e938`). No merge, no deploy.
- **Conflicting REMOTE findings, resolved on evidence.** Two reviews disagreed on the verdict but agreed on the mechanism. One reproduced `F5 → stale bundle → strict-CSP mismatch → hundreds of first-party CSP violations → broken UI`, while `Ctrl+F5` was clean; the other did not reproduce the visible failure in its final run but independently confirmed the stale-cache mechanism. Treated as a real blocker because the mechanism is proven, and the reproduction is inherently timing-dependent.
- **Root cause (proven).** `public/sw.js` — the file Cloudflare Pages actually serves for `/sw.js` (the worker's own `/sw.js` route is excluded from the worker by the generated routes config, so the static copy wins) — served `/static/*` **cache-first under a fixed cache name** while the asset URLs are unversioned (`/static/app.js`, `/static/styles.css`). A new deployment therefore kept serving the previous bundle, and an ordinary reload combined fresh HTML (new per-request CSP nonce) with stale JS/CSS. The violations were a *symptom* of the stale bundle, not of the current code.
- **Fix — and why a version bump is NOT it.** `/static/*` is now **network-first with a cache fallback**, so a deployed asset is always picked up by a normal navigation/reload and the cache survives purely as the offline fallback. This is version-independent: renaming the cache (v2 → v3, done as well for hygiene) would have been recreated by the next deployment, because nothing about a cache name changes when only built assets change. Also: install warms with `cache: 'reload'` (an install can never capture a stale copy), every successful response refreshes the cache, `activate` deletes every cache the running version does not own, and `skipWaiting()` + `clients.claim()` make a new worker take over immediately. `public/sw.js` and the worker-served `src/config/pwa.ts` were drifted copies; both now carry the same lifecycle and a test fails if they diverge. Not cached: cross-origin, `/api/*`, media/stream paths, range requests, non-200, and `no-store`.
- **CSP was NOT weakened.** No `unsafe-inline`, no relaxed directives, no suppression, no nonce bypass. `style-src` stays nonce-only. Audit result: the codebase writes **no** style attribute at runtime; the only runtime writes are CSSOM (`style.cssText` / `setProperty`) and `data-csp-style`, neither of which `style-src` blocks — `SettingsUI` performs no style writes at all. New tests pin both facts so the symptom cannot come back through a runtime style write.
- **B6 avatar click semantics (the second REMOTE report).** The markup was correct but the *behaviour* was not: the delegated `__navigateProfile` never cancelled the default action, so on a surface that is already an anchor the parent's own navigation raced it (card → `/competition/:id`, messages → the conversation). Fixed centrally in the dispatcher: the handler now receives the event and calls `preventDefault()`, and `stopPropagation` is honoured once via `data-csp-stop` in the shared `runHandler` (rather than duplicated per surface). A real bug was also found and fixed while doing this: the new keyboard path passed only the event, so it landed in the handler's *username* argument — the declared args are now parsed by a shared `parseArgs` used by both the click and keydown paths.
- **Keyboard.** Non-native delegated targets (the `span[role=link]` avatar used where an `<a>` would nest invalidly) are now activated by Enter/Space in the dispatcher, excluding natively activatable elements so a real `<button>`/`<a>` cannot double-fire.
- **Avatar surfaces audited.** Linked (identity known): shared competition card (thumbnail + name row), competition detail, messages dropdown, messages-page conversation rows, `chatUserAvatar` header, My Requests, Search/Explore user results (already the whole card's link). Already correct, untouched: Home user results, InvitePanel. **Not linked by design:** donate/supporter rows (donor names are not Dueli account identities) and the live room (no participant avatar UI with a known identity — only an unused `avatar?` type field). No decorative/placeholder avatar was linked.
- **B7 `ORDER BY RANDOM()` — proven, deliberately NOT changed.** The report is correct: `GET /api/competitions` → `CompetitionController.list` → `CompetitionModel.findByFilters` ends in `ORDER BY RANDOM() LIMIT ? OFFSET ?`, which is exactly the endpoint B7 pages. A characterisation test against the real migrations proves consecutive `limit/offset` pages can overlap. Fixing it means changing browse ordering, i.e. product/ranking behaviour, which is out of scope here and is a Lead decision — so it is reported, not silently changed. (This matches the long-standing audit note in `docs/archive/ultimate_forensic_audit.md`.)
- **Tests**: `tests/ui/service-worker-cache-lifecycle.test.ts` (7 — executes the real worker source against a fake Cache API: new deployment wins on an ordinary reload with an old bundle cached, cache refresh, offline fallback still served, previous caches purged, nothing-unsafe cached, both SW copies in sync, no CSP weakening); `tests/ui/b6-avatar-click-navigation.test.ts` (6 — real dispatcher driven through stubbed DOM: card avatar → profile not competition, messages avatar → profile not conversation, Enter/Space, no double-activation, no broken link from a non-username); `tests/api/b7-explore-pagination-ordering.test.ts` (2 — evidence only); plus 3 new CSP runtime guards. `npm test` **727/727 ✅** · `npx tsc --noEmit` ✅ · `npm run build` ✅.
- **Not changed**: no backend/session/schema/migration, no middleware, no dependency, no route, no financial/ads/recommendation-scoring change, no country-menu (PR #67) change.


## 2026-09-26 — UI/Auth stabilization batch B1–B7 (branch `fix/ui-auth-stabilization-batch`)

- **Base / HEAD**: base `ff3e938` (origin/main, PR #67 merge) · branch `fix/ui-auth-stabilization-batch` · no merge, no deploy.
- **Why**: post-R1 acceptance defects found in real use — My Competitions stuck on "login required" or an endless spinner after a successful login, RTL/LTR flipping on client rerender, a wrong-facing "recorded" glyph, flat-purple buttons next to the approved gradient, a profile page that visually diverged from the rest of the platform, real-user avatars that were not clickable to their profile, and Explore "view all" reloading the same first batch.
- **What changed**
  - **B1 auth/session**: modal login now publishes one `dueli:auth-success` event (`AuthService.emitAuthSuccess`) that page scripts subscribe to — My Competitions refreshes without a reload; `loadCompetitions` always terminates in a definite state (`content` / `empty` / `login-required` / recoverable `error` + retry) and never leaves a spinner; `checkAuth` is bounded (10 s, client-side only) and only a genuine **401** clears the session — 429/5xx/network/timeout keep it; logout is unchanged and still clears state.
  - **B2 language**: precedence is now URL `?lang=` → `lang` cookie → `user.language` → browser, so a late-arriving `user.language` cannot flip an already-rendered page; `State.lang` setter also syncs `document.documentElement.dir/lang` through one helper.
  - **B3 RTL recorded badge**: shared `.recorded-play-rtl` rule (rotate, not a scale flip) in `styles.css`, applied by the shared `getCompetitionCard` only — ar points left, en unchanged.
  - **B4 gradient**: the already-approved `from-purple-600 to-indigo-600` treatment is centralised in `src/shared/constants.ts` (`DUELI_PRIMARY_GRADIENT` / `_BTN` / `_TAB_*`) and used by the My Competitions active tab, login CTA, error retry and empty-state CTA. No new gradient invented; secondary/destructive/neutral untouched.
  - **B5 profile**: profile-specific data/actions unchanged; the user competitions list now renders through the shared `getCompetitionCard(...)` on the platform grid instead of the legacy inline card.
  - **B6 avatar links**: new shared `src/shared/components/user-avatar.ts` (`getUserAvatarLink` / `getProfilePath`) built on the canonical `/profile/:username` route — an id is never substituted. Applied to the shared competition card, the competition detail page and the messages dropdown. Where the surface is already an anchor, a CSP-safe `__navigateProfile` action is used instead of an invalid nested `<a>`; avatars without a valid username stay unlinked.
  - **B7 progressive loading**: Explore competitions and users now page with real `limit`/`offset` (confirmed supported by `GET /api/competitions` and `/api/search/users`), append de-duplicated batches, and drop the "view all" full-page reload. Loading / end / error+retry states are explicit, and a keyboard-reachable "load more" button is always present as the IntersectionObserver fallback.
- **What did NOT change**: no backend/session semantics, no schema or migration, no middleware, no dependency, no new route, no financial/ads/recommendation/ranking change, no synthetic-data policy change, no country-menu (PR #67) behaviour change, CSP model preserved (no inline handlers).
- **Tests**: `tests/ui/b1-my-competitions-auth.test.ts` (5), `tests/ui/b1-authservice-session.test.ts` (7), `tests/ui/b2-language-direction.test.ts` (4), `tests/ui/b3-b4-b6-shared-card.test.ts` (12), `tests/ui/b5-profile-shared-card.test.ts` (5), `tests/ui/b7-explore-progressive-loading.test.ts` (5), plus helper `tests/helpers/dom-harness.ts` (executes the pages' real inline scripts against a minimal DOM). `npm test` **709/709 ✅** · `npx tsc --noEmit` ✅ · `npm run build` ✅. CSP + RTL/dark/mobile suites re-run and green (two pre-existing guards caught real issues in this batch: a `style=""` in MessagesUI and a forbidden `scaleX(-1)`, both fixed here).
- **Deferred to R2.2 (unchanged, out of scope)**: the R1.4 finding that the settings form may submit `display_name` / `bio` which the settings backend contract does not accept — not part of the current Profile contract and not proven by a test here.
...
- `[2026-09-25] [PR#67]` — country menu dropdown regression fix: `Menu.toggleCountry` now takes the panel out of flow (`position:fixed`, `right/bottom:auto`, `transform:none`) BEFORE removing `hidden`, and `computeCountryMenuPosition` anchors it strictly below the globe button (`top = triggerBottom + COUNTRY_MENU_TRIGGER_GAP`, 8px) while keeping the 16px viewport safe margin and the narrow-viewport centered fallback. Root cause: the in-flow panel re-measured the header row and shifted the globe button, so the menu covered it. Tests: 2 new regression tests in `tests/ui/header-home-ui-runtime.test.ts` (below-trigger in RTL/LTR/narrow, plus an order assertion that `position:fixed` is applied before `hidden` is removed) and the existing mock gained the real `#countryButton` anchor. base=387f03d, branch=fix/country-menu-position, no merge/deploy, R1 NOT closed by this work. / executor: local agent / verification: `npm test` 671/671, tsc clean, build OK
