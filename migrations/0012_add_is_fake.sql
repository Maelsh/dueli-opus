...
-- NOTE: Column may already exist, so we use IF NOT EXISTS pattern

-- Check if column exists first (SQLite doesn't support IF NOT EXISTS for columns)
-- If you get "duplicate column" error, just skip this migration
...
-- Create index for filtering fake data (ignore if exists)
