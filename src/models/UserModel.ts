...
export class UserModel extends BaseModel<User> {
    ...
    /**
     * Set user as busy
     * تعيين المستخدم كمشغول
     */
    async setBusy(userId: number, competitionId: number): Promise<boolean> {
        const result = await this.db.prepare(`
            UPDATE users 
            SET is_busy = 1, 
     ...
     * تحرير المستخدم
     */
    async setFree(userId: number): Promise<boolean> {
        const result = await this.db.prepare(`
            UPDATE users 
            SET is_busy = 0, 
    ...
    /**
     * Check if user is available (not busy)
     * التحقق من توفر المستخدم
     */
    async checkAvailability(userId: number): Promise<{ available: boolean; reason?: string }> {
        const user = await this.findById(userId);
        
        if (!user) {
            return { available: false, reason: 'المستخدم غير موجود' };
        }
        
        if (user.is_busy) {
            return { 
                available: false, 
                reason: 'المستخدم مشغول في منافسة أخرى' 
            };
        }
     ...
     * تحديث آخر نشاط
     */
    async updateLastActive(userId: number): Promise<boolean> {
        const result = await this.db.prepare(`
