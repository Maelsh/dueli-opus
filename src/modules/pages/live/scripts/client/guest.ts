...
export function getGuestScript(lang: Language): string {
    ...
    return `
        ...
        // إنهاء المنافسة صراحةً (يُستدعى فقط من زر الإنهاء)
        window.endAndLeaveCompetition = async function() {
            window.disconnect();
            const compId = urlCompId || (document.getElementById('compIdInput') ? document.getElementById('compIdInput').value.trim() : null);
            const lang = urlParams.get('lang') || 'en';
            if (!compId) return;
            try {
                await fetch('/api/competitions/' + compId + '/end', {
                    method: 'POST',
                ...
                log('⚠️ Could not update status', 'warn');
            }
            setTimeout(function() {
